...
`v0.0.1` 的部分头文件为了兼容早期单测仍保留少量顶层 `pi::Type` using alias；新代码统一使用 canonical layer namespace。该 alias 不视为未来 v1 稳定 API，最迟在 `v0.3.0` API freeze 时统一审计。
...
这些类型可以脱离 Agent 独立使用，因此必须属于 `ai`，而不是塞在 `agent` 下。

### 6.2 agent 层

`include/pi/agent/message.hpp` 在 `ai::Message` 的基础上增加应用层消息：
...
真正发给模型时，后续 Agent 会在边界处把 AgentMessage 降维成 LLM Message。
...
`include/pi/ai/events.hpp` 描述一个 AssistantMessage 如何流式生成：
...
其消费者可以是 Agent，也可以是一个完全不使用 Agent 的独立 Provider 客户端，因此归属于 `ai`。
...
`message_update` 可以嵌入 `ai::AssistantMessageEvent`，因此依赖方向自然是：

```text
agent -> ai
```

而不是把两层事件放在同一个 header 中。
...
Tau 只能帮助理解，不能决定字段名、event type 或 session 语义。
...
- 值语义；
- 不需要到处 `new` / `unique_ptr`；
- `std::visit` 能让新增 alternative 时更容易发现漏处理分支；
- 与 pi 的判别联合心智模型直接对应。

### 8.4 optional 字段

本项目对 nlohmann/json 3.11.3 做过实际验证，因此规则写死为：

- 简单字段可以使用 NLOHMANN 宏；
- 含 `std::optional` 的结构手写 `to_json/from_json`；
- `nullopt` 序列化为**字段省略**，而不是写 JSON `null`；
...
Abort/cancel 是 pi 架构的一等能力，未来会贯穿：
...
因此 `v0.0.1` 就建立 public abstraction：
...
本版本只声明它是项目自己的 C++17 cancellation abstraction，不宣称与 `std::stop_token` 完全等价。

已知需要在 `v0.0.2`、接入真实 HTTP 前完成的并发加固：

- request 锁内只做状态转换与 callback snapshot；
- callback 解锁后执行；
- unregister/request/destructor race 有确定语义和测试。

---

## 10. fixture 与兼容证据

`v0.0.1` 保留三个手工 JSONL fixture：
...
这些 fixture **不是 pi `v0.80.0` 真实兼容证明**。

从 `v0.0.2` 开始建立真正的 compatibility gate：
...
### 11.2 SDK consumer smoke

`tests/consumer/test_sdk_headers.cpp` 使用 canonical public include：
...
```cpp
pi::ai::UserMessage
pi::agent::AgentMessage
pi::ai::CancellationToken
pi::coding_agent::CodingAgent*
```

这个测试的意义是保证三层 SDK 边界是**编译器可验证的约束**，而不是只写在文档里的目录规划。
...
- [ ] canonical SDK consumer smoke 编译运行通过。
...
nlohmann/json 3.11.3 在本项目实际使用下不能直接依赖“optional 自动序列化”的假设，因此含 optional 的 DTO 明确手写 serializer。这也让字段省略/null 语义更可控。

### 13.2 CMake

- 无源 STATIC library 在部分 CMake 版本上不可行；未有源码的 coding-agent 在本版本使用 INTERFACE target。
- FetchContent 下第三方 target 名必须以实际生成结果为准。
- MSVC 多配置生成器需要显式处理 configuration。
- 第一方 warning-as-error 与第三方 SYSTEM include 必须隔离。

### 13.3 三平台从第一天开始是必要的

Windows VS 多配置、libstdc++ 与 doctest 的细节差异都说明：跨平台不能等产品功能写完以后再补。

---

## 14. 下一步：v0.0.2

由于 SDK 目录、public header 和 CMake target 边界已经在 `v0.0.1` 完成，`v0.0.2` 不再做一次结构性搬家。
...
这样从第一条真实网络请求开始，Provider 就已经工作在正确的 SDK 边界中，而不是先写完功能再重构。
