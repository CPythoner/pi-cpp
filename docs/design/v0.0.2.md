...
真实 T0 harness 通过本地 HTTP/SSE server 调用**public** `OpenAICompatibleProvider`，因此同时覆盖真实 HTTP adapter、SSE parser、delta merger 和 EventStream，而不是绕过 transport 直接调用 private decoder。
...
截至当前 feature branch，功能面不再扩张。剩余工作只做：

1. 等待最后的文档/fixture 提交在三平台 CI 再次收口；
2. 最终 diff audit，确认没有 private HTTP 类型泄漏和超出 v0.0.2 scope 的功能；
3. 更新 changelog / closeout 记录；
4. 合并 `feature/v0.0.2` 后再开始 `v0.0.3` Agent runLoop。
